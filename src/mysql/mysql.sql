...
(
	...
	UNIQUE KEY (us_email),
	FOREIGN KEY (person_id) REFERENCES people(id_person)
);
...
BEGIN
	SELECT id_product, name_product, p.link, description, stock, price, p.picture, p.status, code_product, c.name_category,
	(SELECT COUNT(fa.id_favorite) FROM favorites fa WHERE fa.product_id = p.id_product ) AS is_favorite
	FROM products AS p
	INNER JOIN categories AS c ON p.category_id = c.id_category
	ORDER BY id_product DESC LIMIT 6;
END
...
BEGIN
	...
	FROM products AS p
	INNER JOIN categories AS c ON p.category_id = c.id_category
	LEFT JOIN favorites AS f ON p.id_product = f.product_id
	WHERE c.link = LINK_CATEGORY;
END
